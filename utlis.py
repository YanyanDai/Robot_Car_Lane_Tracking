...
def thresholding(img):
    ...
    lowerWhite = np.array([0,0,123])
    upperWhite = np.array([179, 191, 255])
    maskWhite = cv2.inRange(imgHsv,lowerWhite,upperWhite)
    return  maskWhite

def thresholding_Stop(img):
    imgHsv = cv2.cvtColor(img,cv2.COLOR_BGR2HSV)
    lowerWhite = np.array([8,89,119])
    upperWhite = np.array([195, 182, 175])
    maskWhite = cv2.inRange(imgHsv,lowerWhite,upperWhite)
    return  maskWhite
...
def getHistogram(img,minPer=0.1,display=False, region=1):
    ...
    if display:
        ...
        #return basePoint, NoLane, imgHist
        return basePoint, imgHist

    #return basePoint,NoLane
    return basePoint

def getHistogramStop(img,display=False,region=1):
    if region == 1:
        histValues = np.sum(img, axis=0)
    else:
        histValues = np.sum(img[img.shape[0] // region:, :], axis=0)
